...
class RangeDetection:
    ...
    def __init__(self):
        ...
        #Create the object to update the world map
    ...
    def update_values(self):
        ...
        for i in range(len(self.lidar_scan.ranges)):
            if (self.lidar_scan.ranges[i] > self.sensor_max):
                ...
                self.lidar_values.append(self.lidar_scan.ranges[i])
    ...
    def get_local_minima_v3(self,scan_dist,scan_range =1.5,include_max = True,include_oor = True,safety_distance = 0.05):
        oor = np.where(scan_dist<=0)[0]
        ...
        if oor.size!=0 and include_oor:
            ...
            oor_dist = np.vstack((oor_dist,np.full((oor_dist.shape),(scan_range))))
    ...
    def path_selection_v2(self):
        ...
        adj_dist[adj_dist >= self.sensor_max] = -1

        print('Scan shape:',adj_dist.shape)
        adj_dist = gaussian_filter1d(adj_dist, sigma)

        actions = self.get_local_minima_v3(adj_dist,scan_range=self.sensor_max,safety_distance=0.3)
